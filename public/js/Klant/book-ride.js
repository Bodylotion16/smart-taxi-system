...
function setupAutocomplete(inputId, listId, isPickup) {
    ...
    input.addEventListener('input', function () {
        ...
        fetch(
            `https://nominatim.openstreetmap.org/search?format=json&countrycodes=SR&q=${encodeURIComponent(q)}`
        )
...
function updateRoute() {
    ...
    if (pickupLatLng && destinationLatLng) {
        ...
        routingControl.on('routesfound', function (e) {
            ...
            berekendeAfstand = (
                routes[0].summary.totalDistance / 1000
            ).toFixed(1);
...
document
    ...
    .addEventListener('click', () => {
        ...
        if (!origin || !destination) {
            ...
            alert(
                'Kies eerst een ophaallocatie en een bestemming via de suggesties.'
            );
        ...
        if (berekendeAfstand == 0) {
            ...
            alert(
                'De route wordt nog berekend of er is geen geldige route gevonden.'
            );
        ...
        berekendeFare = (
            berekendeAfstand * TARIEF_PER_KM
        ).toFixed(2);
    ...
    .addEventListener('submit', async e => {
        ...
        try {
            ...
            if (result.success) {
                ...
                const idParam =
                    encodeURIComponent(result.bookingId);
